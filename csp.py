...
class CSP:
    ...
    def fill_cell_to_groups(self):
        ...
        for cell in self.cell_to_groups:
            self.cell_to_groups[cell] = [group_num for group_num, group in enumerate(self.groups) if cell in group]
    ...
    def satisfies_sum_constraint(self, group: typing.List[typing.Tuple[int, int]], sum_constraint: int) -> bool:
        ...
        if sum(self.grid[row_idx, col_idx] for row_idx, col_idx in group) <= sum_constraint:
            return True
        else:
            return False
    ...
    def satisfies_count_constraint(self, group: typing.List[typing.Tuple[int, int]], count_constraint: int) -> bool:
        ...
        cell_values, cell_counts = np.unique([self.grid[row_idx, col_idx] for row_idx, col_idx in group], return_counts=True)
        for value, count in zip(cell_values, cell_counts):
            if value != 0 and count > count_constraint:
                return False
        else:
            return True
    ...
    def satisfies_group_constraints(self, group_indices: typing.List[int]) -> bool:
        ...
        for index in group_indices:
            if not (self.satisfies_sum_constraint(self.groups[index], self.constraints[index][0]) and
    ...
    def search(self, empty_locations: typing.List[typing.Tuple[int, int]]) -> np.ndarray:
        ...
        if len(empty_locations) > 0:
            for number in self.numbers:
                ...
                grid = self.search(empty_locations[1:])
